...
if (!isset($_GET['userid'])) {
  // If the userid is not provided in the URL, redirect to the index page
  header("Location: index.php");
  exit();
}
...
// Assuming $pdo is the PDO connection object, you need to make sure it is properly set.
// Assuming $pdo is the PDO connection object, you need to make sure it is properly set.
try {
...
// Prepare the SQL query to find the user's group based on the userid
$query = "SELECT id FROM `groups` WHERE userid = :userid";
$stmt = $pdo->prepare($query);
$stmt->bindParam(':userid', $_SESSION['userid']); // Use the userid from the session
$stmt->execute();
...
if ($group) {
  // Display the group information or perform any actions you need
  "User belongs to group: " . $group['id'];
} else {
  // If no group is found for the user, handle it as per your requirement
  // For example, redirect to a generic group page or display an error message
...
<body id="body">
    ...
    <div class="row justify-content-center">
      <div class="col-md-6">
        <div class="card">
          ...
          <h3 class="mb-3">User Id - <?php echo $userId; ?></h3>
          <div class="container">
            <table class="styled-table">
              ...
              <tbody>
                ...
                foreach ($groups as $group) {
                    ...
                    <td><a href='product.php?groupid=" . urlencode($group["id"]) . "&groupname=" . urlencode($group["groupname"]) . "' class='custom-link'>" . $group["id"] . "</a></td>
                    <td><a href='product.php?groupid=" . urlencode($group["id"]) . "&groupname=" . urlencode($group["groupname"]) . "' class='custom-link'>" . $group["groupname"] . "</a></td>
                    <td><a href='product.php?groupid=" . urlencode($group["id"]) . "&groupname=" . urlencode($group["groupname"]) . "' class='custom-link'>
                    <button class='btn-blue'>Open</button></a></td>
                    <td><a href='group.php?gn=" . urlencode($group["groupname"]) . "' onclick='return confirm(\"Are you sure you want to delete this group?\");' class='btn-red'>Delete</a></td>
                  </tr>";
                }
                ?>
              </tbody>
  ...
  <div class="settings-page" style="display: none;">
    ...
    <button class="btn-red" onclick="window.location.href = 'index.php?logout=true'" >Log out</button>
  </div>
